...
class CouponsController extends Controller
{
	...
	{
		$Coupon = Coupon::orderBy('id', 'asc');
		if (Auth::user()->role_id != 1) {
			$Coupon = $Coupon->where('user_id', Auth::user()->id);
		}
		$Coupon = $Coupon->get();
	...
	public function edit($id)
	{
		...
		$discounted_products = Product::query();
		if (Auth::user()->role_id != 1) {
			$discounted_products = $discounted_products->where('user_id', Auth::user()->id);
		}
		$discounted_products = $discounted_products->where('coupon_id', $Coupons->id)->orderBy('created_at', 'ASC')->get();
