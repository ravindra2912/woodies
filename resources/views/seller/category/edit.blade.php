				...
				<div class="col-md-6 col-sm-10">
					<div class="card card-primary">
						...
						<form method="POST" id="category_form" enctype="multipart/form-data">{{ csrf_field() }}
							<input type="hidden" id="form_url" value="{{ route('category.update',$categoryData->id) }}">
							...
							<div class="card-body">
								...
								<div class="form-group">
									...
									<select name="parent_id" class="form-control">
										...
										<option value="{{ $cat->id }}" {{ $cat->id == $categoryData->parent_id ? 'selected':'' }}>{{ $cat->name }}</option>
										@endforeach
									</select>
								</div>
								...
								<div class="form-group">
									<label class="required">Category Name</label>
									<input type="text" name="category_name" id="category_name" class="form-control" placeholder="Category Name" value="{{ (isset($categoryData->name) && !empty($categoryData->name)) ? $categoryData->name : '' }}" required>
								</div>

								<div class="form-group">
									<label>Category Image</label>
									<br />
									<img src="{{ getImage($categoryData->image) }}" style="height:120px; width:120px;">
								</div>

								<div class="form-group mt-2">
									<label class="required">Change Category Image</label>
								...
								<div class="form-group">
									...
									<br />
									<img src="{{ getImage($categoryData->banner_img) }}" style="height:120px; width:120px;">
								</div>
								...
								<div class="form-group">
									...
									<select name="status" id="status" class="form-control" id="status" required>
										<option value='Active' {{ (isset($categoryData->status) && $categoryData->status == "Active") ? 'selected' : '' }}>Active</option>
										<option value='Inactive' {{ (isset($categoryData->status) && $categoryData->status == "Inactive") ? 'selected' : '' }}>Inactive</option>
									</select>
								</div>
								...
								<div class="form-group 6">
									...
									<textarea class="form-control" name="SEO_description" placeholder="SEO Description">{{ $categoryData->SEO_description }}</textarea>
								</div>

								<div class="form-group ">
									<label class="">SEO Tags (,)</label>
									<textarea class="form-control" name="SEO_tags" placeholder="SEO Tags">{{ $categoryData->SEO_tags }}</textarea>
								</div>
